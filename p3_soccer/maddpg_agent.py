...
class mAgent():
    ...
    def load(self):
        for posit in range(self.num_agents):
            ...
            self.agents[posit].actor_local.load_state_dict(torch.load('checkpoints/checkpoint_actor_%d.pth'%(posit), map_location='cpu'));
            self.agents[posit].critic_local.load_state_dict(torch.load('checkpoints/checkpoint_critic_%d.pth'%(posit), map_location='cpu'));
